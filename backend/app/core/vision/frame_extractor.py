...
class FrameExtractor(BaseProcessor):
    ...
    def extract_at_timestamps(
        self, video_path: str, timestamps_s: list[float]
    ) -> list[tuple[int, float, np.ndarray]]:
        """Extract frames at specific timestamps using CAP_PROP_POS_MSEC seeking.

        Parameters
        ----------
        video_path : str
            Path to the video file.
        timestamps_s : list of float
            Timestamps in seconds at which frames should be extracted.

        Returns
        -------
        list of (frame_number, timestamp_s, rgb_frame)
            Sorted by timestamp.  Timestamps within 50 ms of each other are
            deduplicated (only the first is kept).  Timestamps outside the
            video duration are skipped.
        """
        path = Path(video_path)
        if not path.exists():
            raise FileNotFoundError(f"Video file not found: {video_path}")
        ...
        try:
            ...
            duration_s: float = total_frames / native_fps

            # Sort and deduplicate timestamps within 50 ms
            sorted_ts = sorted(timestamps_s)
            deduped: list[float] = []
            for ts in sorted_ts:
                if deduped and abs(ts - deduped[-1]) < 0.05:
                    continue
                deduped.append(ts)
            ...
            for ts in deduped:
                if ts < 0 or ts > duration_s:
                    logger.debug("Timestamp %.3f s is outside video duration, skipping.", ts)
                    continue

                cap.set(cv2.CAP_PROP_POS_MSEC, ts * 1000.0)
                ret, frame = cap.read()
                if not ret:
                    logger.debug("Could not read frame at timestamp %.3f s", ts)
                    continue

                frame_number = int(cap.get(cv2.CAP_PROP_POS_FRAMES)) - 1
                rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                frames.append((frame_number, ts, rgb_frame))

            logger.info(
                "Extracted %d frames at specific timestamps from %s",
